...
public class Main {
    public static void main(String[] args) {
        ...
        //Hướng đối tượng (Object-Oriented Programming - OOP) là một phương pháp lập trình mà trong đó, chương trình được tổ chức dựa
        // trên các đối tượng và dữ liệu thay vì hàm và logic. OOP có bốn tính chất quan trọng:
        //
        //1. **Tính đóng gói (Encapsulation)**: Tính đóng gói là cách để che dấu dữ liệu và phương thức làm việc với dữ liệu đó từ bên ngoài.
        // Trong Java, bạn có thể sử dụng các từ khóa `private`, `protected`, và `public` để kiểm soát quyền truy cập.
        //
        //2. **Tính kế thừa (Inheritance)**: Tính kế thừa cho phép một lớp kế thừa các thuộc tính và phương thức của một lớp khác.
        // Lớp kế thừa được gọi là lớp con, và lớp được kế thừa được gọi là lớp cha.
        //
        //3. **Tính đa hình (Polymorphism)**: Tính đa hình cho phép một đối tượng hoạt động như một đối tượng của một lớp khác.
        // Điều này có thể được thực hiện thông qua việc ghi đè phương thức (method overriding) hoặc ép kiểu đối tượng (type casting).
        //
        //4. **Tính trừu tượng (Abstraction)**: Tính trừu tượng là quá trình tạo ra một lớp trừu tượng bằng cách định nghĩa các thuộc tính và
        // phương thức cơ bản mà một lớp cần phải có, nhưng để cho lớp con xác định cách thực hiện các phương thức đó.
        // Trong Java, tính trừu tượng được thực hiện thông qua các lớp trừu tượng (abstract class) và giao diện (interface).
        ...
        //
        //2. `default` (không sử dụng modifier): Có thể truy cập từ bên trong lớp chứa nó và từ các lớp khác trong cùng một package.
        // Điều này có nghĩa là các lớp trong cùng một package có thể truy cập các thành phần `default`.
        //
        //3. `protected`: Có thể truy cập từ bên trong lớp chứa nó, từ các lớp khác trong cùng một package,
        // và từ các lớp con của lớp chứa nó, kể cả khi chúng nằm trong các package khác nhau.
        //
        ...
        //
        //1. `Class`: Được xem như là một khuôn mẫu (template) để tạo ra các đối tượng. Nó định nghĩa một nhóm các đối tượng có chung các thuộc tính (đặc điểm)
        // và phương thức (hành vi). Ví dụ, bạn có thể tạo một `class` tên là `Car` với các thuộc tính như `color`, `brand`, `model`
        // và các phương thức như `drive()`, `brake()`.
        //
        //```java
...
//            Phương thước
        ...
        //```
        //
        //2. `Instance`: Là một thực thể cụ thể của một `class`. Khi bạn tạo một `instance` của một `class`, bạn đang tạo một đối tượng cụ thể
        // dựa trên khuôn mẫu của `class` đó. Mỗi `instance` có thể có các giá trị khác nhau cho các thuộc tính được định nghĩa trong `class`.
        //
        //```java
        Car myCar = new Car();
        ...
        //```
        //
        //Trong ví dụ trên, `myCar` là một `instance` của `class` `Car`. Bạn có thể thiết lập các thuộc tính và
        // gọi các phương thức của `myCar` dựa trên những gì đã được định nghĩa trong `class` `Car`.
        ...
        //
        //- Sử dụng `Abstract` khi bạn muốn tạo một lớp cơ sở mà các lớp khác có thể kế thừa từ đó,
        // và lớp cơ sở đó có một số phương thức mà bạn muốn cung cấp cài đặt mặc định.
        //
        ...
        //2. `static`: Phương thức `static` không phải là một phần của đối tượng, mà là một phần của lớp.
        // Do đó, phương thức `static` không thể được ghi đè. Tuy nhiên, phương thức `static` có thể được "ẩn" trong lớp con
        // nếu lớp con định nghĩa một phương thức `static` khác với cùng tên và cùng tham số.
...
//            }
        ...
        //```java
        class MyClassHaveConstructor{
            private int x;
            public MyClassHaveConstructor(int x){
                this.x = x;
//                This tham chiếu đến thuộc tính 'x' của lớp hiện tại
            }
